...
print("="*80)
print("CREATING GEOMETRIC MEAN V2 VISUALIZATION WITH DATA EXPORT")
print("="*80)
print("\nLoading geometric mean v2 data...")

# Define paths
input_dir = 'geometric_mean_output2'
output_dir = 'geometric_mean_output2'

geom_df = pd.read_csv(os.path.join(input_dir, 'geometric_mean_v2_pathway_fluxes.csv'))
...
sns.heatmap(heatmap_data,
            ...
            vmin=0,
            vmax=1,
            cbar_kws={'label': 'Pathway Capacity (0-1 scale)', 'shrink': 0.8},
            linewidths=0.5,
            linecolor='white',
            ax=ax1)

ax1.set_title('A. Main Metabolic Pathways by Tissue (Geometric Mean v2)',
...
sns.heatmap(heatmap_data,
            ...
            vmin=0,
            vmax=1,
            cbar_kws={'label': 'Pathway Capacity (0-1 scale)'},
            linewidths=0.5,
            linecolor='white',
            ax=ax_a)
ax_a.set_title('A. Main Metabolic Pathways by Tissue (Geometric Mean v2)',
              fontsize=14, fontweight='bold', pad=20)
...
sns.heatmap(heatmap_data_aa,
            cmap='viridis',
            vmin=0,
            vmax=0.5,  # AA catabolism typically lower
            cbar_kws={'label': 'Pathway Capacity (0-1 scale)', 'shrink': 0.8},
            linewidths=0.5,
            linecolor='white',
...
ax2.set_title('B. Amino Acid Catabolism Pathways by Tissue',
              fontsize=14, fontweight='bold', pad=20)
...
sns.heatmap(heatmap_data_aa,
            ...
            yticklabels=[p.replace('_catabolism', '').replace('_', ' ') for p in aa_pathways],
            ax=ax_b)
ax_b.set_title('B. Amino Acid Catabolism Pathways by Tissue',
              fontsize=14, fontweight='bold', pad=20)
...
legend_elements_g = [Patch(facecolor='crimson', alpha=0.7, label='Male > Female'),
                    Patch(facecolor='steelblue', alpha=0.7, label='Female > Male')]
ax_g.legend(handles=legend_elements_g, loc='lower right', fontsize=9)
plt.tight_layout()
...
# ==================== Overall Title ====================
fig.suptitle('Geometric Mean v2 Method: Comprehensive Metabolic Pathway Analysis\n' +
            f'245 Mouse Tissue Samples | 29 Pathways | 0-1 Normalized Scale',
            fontsize=16, fontweight='bold', y=0.995)

# Save figure
plt.savefig(os.path.join(output_dir, 'geometric_mean_v2_visualization.png'), dpi=300, bbox_inches='tight')
print("\n✓ Saved comprehensive visualization to geometric_mean_v2_visualization.png")
plt.close()
