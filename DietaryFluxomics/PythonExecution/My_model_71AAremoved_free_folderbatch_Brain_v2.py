...
if __name__ == '__main__':
    ...
    model.set_configuration(ncpus = 8) #Number of local CPUs for Parallel python
    #
    # Loading metabolite state from text file to constrain flux vector
    #
    state_dic = model.load_states("My_status_v71AAremoved_Brain_v3.csv", format = 'csv')
    model.set_constraints_from_state_dict(state_dic)
    model.update()

    source_dir2 = 'MDV_Brain/'
    dest_dir2 = 'Results_Brain/'
    carbon_ratio = pd.read_csv('Brain_Carbon_Source_v2.csv', index_col=0)
    ...
    for file in files2:
        if file.endswith(".DS_Store"):
            ...
            print(file)
            result_path = dest_dir2 + file
            Glc_C12 = carbon_ratio.loc['Glc_C12', file]
            Glc_C13 = carbon_ratio.loc['Glc_C13', file]
            Lac_C12 = carbon_ratio.loc['Lac_C12', file]
            Lac_C13 = carbon_ratio.loc['Lac_C13', file]
            carbon_source1 = model.generate_carbon_source_template()
            carbon_source1.set_all_isotopomers('SubsCO2', [0.99, 0.01])
            carbon_source1.set_each_isotopomer('SubsGlc', {'#000000': Glc_C12, '#111111': Glc_C13}, correction='yes')
            ...
            carbon_source1.set_each_isotopomer('SubsLac', {'#000': Lac_C12, '#111': Lac_C13}, correction='yes')
            ...
            mdv_observed1 = model.load_mdv_data(source_dir2 + file)
            ...
            method_local = "SLSQP"
            state1, flux_opt1 = model.generate_initial_states(1000, 8, method="parallel")
